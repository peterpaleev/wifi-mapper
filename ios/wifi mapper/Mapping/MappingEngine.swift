...
final class MappingEngine: @unchecked Sendable {
    ...
    func begin() { queue.async {
        ...
        do {
            ...
            self.poses.removeAll();self.points.removeAll();self.pending.removeAll();self.descriptors.removeAll();self.boot=nil;self.lastSyncT1=0;self.lastHeatCount = -1;self.heatVersion += 1
    ...
    func select(_ id: UInt16?) { queue.async { if self.state.selectedAP != id { self.state.selectedAP=id;self.lastHeatCount = -1;self.heatVersion += 1;self.state.revision += 1 } } }
    func options(heat: Bool,height: Double?) { queue.async { self.heatEnabled=heat;self.heightCenter=height;self.lastHeatCount = -1;self.heatVersion += 1 } }
    ...
    func load(_ folder: URL) { queue.async {
        ...
        do {
            ...
            self.state.selectedAP=data.aps.keys.sorted().first;self.state.rawCount=data.rawCount;self.state.alignedCount=data.points.count;self.state.unalignedCount=max(0,data.rawCount-data.points.count);self.state.position=data.poses.last?.position ?? .zero
    ...
    private func publish() {
        ...
        if heatEnabled {
            ...
            if lastHeatCount != selected.count,now-lastHeat>=1,!heatBusy {
                ...
                let height=heightCenter,version=heatVersion
                heatQueue.async {
                    let result=HeatmapBuilder.generate(selected,heightCenter:height)
                    self.queue.async { self.heatBusy=false;if version==self.heatVersion && self.heatEnabled { self.state.heat=result } }
                }
            }
...
final class ARTracker: NSObject, ARSessionDelegate, @unchecked Sendable {
    ...
    func start() {
        let config=ARWorldTrackingConfiguration();config.worldAlignment = .gravity;config.planeDetection=[.horizontal]
        delegateQueue.async { self.lastFrame=0;self.segment=0;self.wasNormal=false }
...
@MainActor final class MappingModel: ObservableObject {
    ...
    init() {
        ...
        #if DEBUG
        if ProcessInfo.processInfo.arguments.contains("--ui-test-fixture") { heatEnabled=true;engine.fixture() }
        #endif
    }
    func start(inspector: InspectorModel) {
        ...
        Task {
            ...
            engine.begin();tracker.start();engine.select(inspector.snapshot.selected);heightCenter=0
    ...
    func completeStop() { stopRequested=false;stopFallback?.cancel();engine.finish();tracker.stop();Task { try? await Task.sleep(for:.milliseconds(300));surveys=SurveyStore.list() } }
    ...
    private func updateOptions() { engine.options(heat:heatEnabled,height:heightSlice ? heightCenter : nil) }
}
