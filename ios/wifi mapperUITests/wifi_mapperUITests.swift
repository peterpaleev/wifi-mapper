...
final class wifi_mapperUITests: XCTestCase {
    ...
    @MainActor func testSpatialMapAndOptions() throws {
        ...
        XCTAssertTrue(app.staticTexts["Layers"].waitForExistence(timeout:5))
        XCTAssertTrue(app.switches["Floor surfaces"].exists)
        XCTAssertTrue(app.switches["Walls"].exists)
        app.switches["Walls"].tap()
        capture("Layer settings")
        app.swipeUp()
        capture("Heatmap settings")
        app.buttons["Done"].tap()
        app.buttons["Saved surveys"].tap()
        XCTAssertTrue(app.navigationBars["Surveys"].waitForExistence(timeout:5))
        app.buttons["Done"].tap()
    }
    @MainActor func testSatelliteFixtureAndLayerSwitch() throws {
        ...
        app.buttons["Map options"].tap()
        app.switches["Apple Maps satellite imagery"].switches.firstMatch.tap()
        app.buttons["Done"].tap()
